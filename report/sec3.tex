...
However, as described in the dataset presentation, they divide territory in different ways: the \texttt{communes} dataset has the precise polygon representing commune border, while \texttt{census} dataset rely on a grid of cells.
This obliged us to perform some form of approximation in order to compute communes population.
...
Where $C$ represents the commune, $G$ represents the cell of the grid and $P$ the population.\\
...
Since we know cells to have an area of \qty{1}{km^2}, we can omit \mintinline{sql}|ST_AREA(g.geom)| from the denominator, substituting it with \num{1000000}, that is \qty{1}{km^2} converted to \si{m^2}.
