...
The dataset distributes population counts into a regular grid of \qty{1}{km^2} cells, offering a highly detailed spatial perspective on demographic distribution.
...
The dataset is based on a grid of cells of \qty{1}{km^2} and contains indicators of accessibility expressed in terms of travel distance and travel time from each grid cell to the nearest healthcare service.
...
\begin{itemize}
	\item driving time from each populated \qty{1}{km^2} cell to the nearest healthcare service
...
\begin{itemize}
	\item \texttt{COMM\_ID}: identification code of the municipality;
	\item \texttt{CNTR\_ID}: here is simply the copy of CNTR\_CODE, but it can also be an internal identification number of Eurostat;
	\item \texttt{CNTR\_CODE}: country code as for ISO 3166-1 alpha-2;
	\item \texttt{COMM\_NAME}: name of the municipality;
	\item \texttt{NAME\_ASCI}: name of the municipality without special characters (ASCII characters);
	\item \texttt{TRUE\_FLAG}: a technical attribute used to distinguish real polygons from those that exist only for cartographic or topological reasons;
	\item \texttt{NSI\_CODE}: municipality code according to the National Statistics Institute;
	\item \texttt{NAME\_NSI}: name of the municipality according to the National Statistics Institute;
	\item \texttt{NAME\_LATN}: name of the municipality written with Latin characters. This is for countries which does not use Latin alphabet (Greece, Bulgaria, etc.);
	\item \texttt{NUTS\_CODE}: NUTS CODE (Nomenclature of territorial units for statistics).
\end{itemize}
